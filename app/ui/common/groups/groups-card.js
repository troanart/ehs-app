...
const GroupsCards = ({ selectedColors, filterType }) => {
  ...
  return (
    <List
      ...
      renderItem={(item) => {
        const studentsGroup = students.filter(
          (student) => student.groupId === item.id
        );

        return (
          <List.Item key={item.id}>
            <Card
              className={`border-l-[15px] text-left relative min-h-[250px] `}
              style={{ borderLeftColor: getBorderColor(item.type) }}
              title={<div className="text-2xl">{item.name}</div>}>
              <IconButton className="absolute top-2 right-3">
                <MoreHorizIcon color="primary" />
              </IconButton>
              <div>
                <p className="inline-block text-lg">Викладач:</p>
                <span className="ml-2 text-sm">{item.teacher}</span>
              </div>
              <div className="border my-2 p-2 rounded-lg border-dashed border-[#1976d2]">
                <div className="flex flex-wrap gap-1 ">
                  {studentsGroup.map((student) => (
                    <Tag
                      key={student.id}
                      className="bg-sky-300 border-none px-3">
                      {student.name}
                    </Tag>
                  ))}
                </div>
                ...
                <Row gutter={16}>
                  <Col span={12}>
                    <Card className="bg-inherit" bordered={true}>
                      <Statistic
                        title="Прибуток"
                        value={new Intl.NumberFormat("uk", {
                          notation: "compact",
